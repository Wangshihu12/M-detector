...
void DynObjFilter::filter(PointCloudXYZI::Ptr feats_undistort, const M3D &rot_end, const V3D &pos_end, const double &scan_end_time)
{
    ...
    if (time_file != "")
        time_out << omp_get_wtime() - clus_before << " "; // rec computation time
    double t3 = omp_get_wtime();
    Points2Buffer(points, index);
    double t4 = omp_get_wtime();
    if (time_file != "")
        time_out << omp_get_wtime() - t3 << " "; // rec computation time
    Buffer2DepthMap(scan_end_time);
    if (time_file != "")
        time_out << omp_get_wtime() - t3 << endl; // rec computation time
    if (cluster_coupled)
    {
        for (int i = 0; i < size; i++)
        {
